...
// --- Функции для отображения ПВЗ и стилей ---
...
$(document).ready(function() {
  ...
  $(document).on('inited:insales:checkout:deliveries', function(e) {
    ...
    if ($deliveryElement.length) {
      ...
      console.warn('⚠️ Элемент способа доставки ID 1499345 не найден.');
    }
  });
});
...
function handlePickupPointSelection($selectedPoint, orderData) {
  ...
  if (orderWeight <= 0) {
    // Пример получения веса из DOM (может отличаться)
    // const weightElement = document.querySelector('[data-weight]'); // Если есть
    // if (weightElement) {
    //   orderWeight = parseFloat(weightElement.value) || 0;
    // }
    // Пока используем фиксированное значение для теста
    orderWeight = 1.0;
    console.log('📦 Вес заказа из DOM (или фиксированный):', orderWeight);
  }
  ...
  calculatePickupDelivery(orderWeight, pointId)
    .then(result => {
      ...
      const deliveryDataForUpdate = {
        ...
        fields_values: [
          // Сохраняем ID и название выбранного ПВЗ как доп. поле
          // ЗАМЕНИТЕ 12345 И 12346 НА РЕАЛЬНЫЕ ID ДОПОЛНИТЕЛЬНЫХ ПОЛЕЙ В INSALES
          // { field_id: 12345, value: pointId.toString() },
          // { field_id: 12346, value: pointTitle }
        ],
        is_external: true, // Указываем, что это внешний способ
        // external_data: { selected_pickup_point_id: pointId } // Произвольные данные
      };
