...
function createJSONPResponse(data, callbackName) {
  ...
  // (базовая проверка, в реальных приложениях может потребоваться более строгая валидация)
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (path === '/api/delivery/calculate' && httpMethod === 'GET') {
      ...
      if (callbackName) {
        ...
        try {
            ...
            statusCode: 200,
            headers: {
                'Content-Type': 'application/javascript', // Важно для JSONP
            },
            body: jsonpBody
            };
        } catch (jsonpError) {
        ...
        // Если callback нет, возвращаем обычный JSON
        console.log('Sending JSON response');
        return {
          statusCode: 200,
          ...handleCORS(), // Устанавливаем CORS заголовки
    ...
    // (Теперь также может обрабатывать JSONP, если пришёл callback, но обычно не используется)
    if (path === '/api/delivery/calculate' && httpMethod === 'POST') {
      ...
      // Проверяем callback и для POST на всякий случай
      const callbackName = requestBody?.callback || event.queryStringParameters?.callback;
      if (callbackName) {
